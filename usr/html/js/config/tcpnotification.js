...
{
	var classNum = ["04030644", "0501", "04030647", "04030648"];
	InitMsgLang(classNum);
	setLanguage(parent.g_configData.langPath, setup + maincontents + "tcpnotification", 
...
function InitSetting()
{
	...
	$("formTcpPushPort:text").numeric();
	$("formTcpPushConnect:text").numeric();
	$("formTcpPushSend:text").numeric();
	$("formTcpPushTime:text").numeric();
 

	$("#formTcpPushIp:text").val(eval(group+ "_NOTIFY_TCPPUSH_ADDRESS"));
	$("#formTcpPushPort:text").val(eval(group+ "_NOTIFY_TCPPUSH_PORT"));
	$("#formTcpPushConnect:text").val(eval(group+ "_NOTIFY_TCPPUSH_CONNECT_TIMEOUT"));
	$("#formTcpPushSend:text").val(eval(group+ "_NOTIFY_TCPPUSH_SEND_TIMEOUT"));
	$("input[name='formTcpPushAliveType'][value='" + eval(group + "_NOTIFY_TCPPUSH_ALIVE_TYPE") + "']:radio").attr("checked", "checked");
	$("#formTcpPushTime:text").val(eval(group+ "_NOTIFY_TCPPUSH_ALIVE_TIMEOUT"));

	$("#formTcpPushIp:text").ipaddress();
	$("#formTcpPushIpAddress .ip_octet").css("border", "0px");
...
{
	var group = g_defaultGroup;
	$("#formTcpPushPort:text").blur(function() {
		var inputValTextTcpPushPort = $("#formTcpPushPort:text").val();
		if(inputValTextTcpPushPort == "")
			return;

		inputValTextTcpPushPort = $("#formTcpPushPort:text").val()-0;
		$("#formTcpPushPort:text").val(inputValTextTcpPushPort);

		if(inputValTextTcpPushPort < 1 || inputValTextTcpPushPort > 65535)
		{
			$("#formTcpPushPort:text").val(eval(group+ "_NOTIFY_TCPPUSH_PORT")).focus();
			alert(GetMsgLang("04030644"));
		}
	});
	$("#formTcpPushConnect:text").blur(function() {
		var inputValTextTcpPushConnect = $("#formTcpPushConnect:text").val();
		if(inputValTextTcpPushConnect < 1 || inputValTextTcpPushConnect > 300)
		{
			$("#formTcpPushConnect:text").val(eval(group+ "_NOTIFY_TCPPUSH_CONNECT_TIMEOUT")).focus();
			alert(GetMsgLang("04030644"));
		}
	});
	$("#formTcpPushSend:text").blur(function() {
		var inputValTextTcpPushSend = $("#formTcpPushSend:text").val();
		if(inputValTextTcpPushSend < 1 || inputValTextTcpPushSend > 300)
		{
			$("#formTcpPushSend:text").val(eval(group+ "_NOTIFY_TCPPUSH_SEND_TIMEOUT")).focus();
			alert(GetMsgLang("04030644"));
		}
	});
	$("#formTcpPushTime:text").blur(function() {
		var inputValTextTcpPushAliveTime = $("#formTcpPushTime:text").val();
		if(inputValTextTcpPushAliveTime < 1 || inputValTextTcpPushAliveTime > 86400)
		{
			$("#formTcpPushTime:text").val(eval(group+ "_NOTIFY_TCPPUSH_ALIVE_TIMEOUT")).focus();
			alert(GetMsgLang("04030644"));
		}
	});

	// Alive Type
	$("input[name='formTcpPushAliveType']").change(function(){
		if($("input[name='formTcpPushAliveType']:checked:radio").val() == "timeout")
		{
			Enable($("#formTcpPushTime:text"));
		}
		else
		{
			Disable($("#formTcpPushTime:text"));
		}
	});
	$("input[name='formTcpPushAliveType']").change();
}
...
function EventBind()
{
	...
	$("#btnApply").button().click(function() {
		...
		QString
			.set_action('update')
			.set_schema('xml')
			.add_list("EVENT.Notify.Tcppush.address", eval(group+ "_NOTIFY_TCPPUSH_ADDRESS"), $("#formTcpPushIpAddress #_octet_1").val() + '.' +
																					$("#formTcpPushIpAddress #_octet_2").val() + '.' +
																					$("#formTcpPushIpAddress #_octet_3").val() + '.' +
																					$("#formTcpPushIpAddress #_octet_4").val())
			.add_list("EVENT.Notify.Tcppush.port", eval(group+ "_NOTIFY_TCPPUSH_PORT"), $("#formTcpPushPort:text").val())
			.add_list("EVENT.Notify.Tcppush.Connect.timeout", eval(group+ "_NOTIFY_TCPPUSH_CONNECT_TIMEOUT"), $("#formTcpPushConnect:text").val())
			.add_list("EVENT.Notify.Tcppush.Send.timeout", eval(group+ "_NOTIFY_TCPPUSH_SEND_TIMEOUT"), $("#formTcpPushSend:text").val())
			.add_list("EVENT.Notify.Tcppush.Alive.type", eval(group+ "_NOTIFY_TCPPUSH_ALIVE_TYPE"), $("input[name='formTcpPushAliveType']:checked:radio").val())
			.add_list("EVENT.Notify.Tcppush.Alive.timeout", eval(group+ "_NOTIFY_TCPPUSH_ALIVE_TIMEOUT"), $("#formTcpPushTime:text").val());
	...
	$("#linkcustomaizeventfmt").button().click(function(){
		ResizePage(700);
	$("#msgfmt_form").dialog("option", "title", GetMsgLang("04030647"));
	...
	*/
	...
	$("#idtext_sysformat").mouseover(function () {
		var example = "count=1&#38prevmask=1&#38currmask=0";
		var sz = setupKWTooltip(gTemperKeywords, example);
...
{	
	...
	if(type == 0)
	{
		msg += "The charactor '@' is reserved by system and it could not be used in format";
	}
	else if(type == 1)	
	{		
		msg += "The message's format is not correct, please check the pairs of '$'";
	}	
	else	
	{		
		msg += "\"";		
		...
		msg += "\"";		
		msg += "is the unsupported keyword!";	
	}	
	alert(msg);
}
