...
(function (CAP, VCA, window, undefined) {
  // Language
  getLangEnvironment("scenechange");
	...
	page.eventCallback =
		{
			...
			submit: function (event) {
				try {
					...
					CAP.ajax.autoSave(window,'VCA.Ch0.Sd');
				} catch (exception) {
			...
			restoreDefaults: function (event) {
				try {
//					var doRestore = confirm("WARNING: This will restore all scene change detection settings to their default values. Are you sure you want to continue?");
					var doRestore = confirm(top.GetMsgLang("04050724"));
			...
			settingChanged: function (event) {
				try {
					   ...
					   ((event.type == 'change') || (event.type == 'keyup' && window.event.keyCode == 13)))
					{
						...
						{
						case 'timeout':
							if(originator.value < 1) originator.value = 0;
							if(originator.value > 10) originator.value = 10;
							break;
						case 'firedarea':
							if(originator.value < 1) originator.value = 0;
			...
			updateAdvancedEnable : function()
			{
				var numElements = $('#manual:checked').toArray().length;
				if(numElements != 0)
					$('.advancedsetting').attr("disabled",false);
				else
					$('.advancedsetting').attr("disabled",true);
			},
	...
	page.methods = {
			init: function ()
			{
				try {
					...
					CAP.logging.verbose('Initialising VCA Scene Change Detection page...');
