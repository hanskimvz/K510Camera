	...
	$.fn.ipaddress = function(params) {
		return $(this).each(function(){
			...
			if (!$this.hasClass('ip-enabled')) {
				...
				//$this.after($('<div class="ip_container" style="display: inline' + (($.browser.msie) ? '' : '-block') + ';"/>').html(octet_html));  $this.addClass('ip-enabled');  }
				$this.after($('<div class="ip_container" style="display: inline-block' + ';"/>').html(octet_html));  $this.addClass('ip-enabled');  }
				if($.browser.msie == true) { if($.browser.version == 7.0) $("div.ip_container").css("display", "inline");  } // ie7
				var isNumeric = function(e){
					if (e.shiftKey) return false;  return (e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105);
				};
				...
				var saveIP = function(el) {  // save value to original input if all octets have been entered
